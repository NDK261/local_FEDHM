@echo off
setlocal

cd /d "%~dp0"

set "PYTHON_EXE=%~dp0.venv39\Scripts\python.exe"
if not exist "%PYTHON_EXE%" set "PYTHON_EXE=%~dp0.venv\Scripts\python.exe"

if not exist "%PYTHON_EXE%" (
    echo [ERROR] Khong tim thay python trong .venv39 hoac .venv
    pause
    exit /b 1
)

set "PORT=50093"
set "EXPERIMENT=splitmix_lvl3_20c_p25_3s_b4_e2_r200_split2612"

echo Running %EXPERIMENT% on port %PORT%
echo Python: %PYTHON_EXE%

"%PYTHON_EXE%" -u -W ignore run_gloo.py ^
  --port %PORT% ^
  --arch ensresnet18_3.15 ^
  --complex_arch master=ensresnet18_3.15,worker=ensresnet18_3.15:ensresnet18_1.05:ensresnet18_0.7,num_clients_per_model=2:6:12 ^
  --partition_data non_iid_dirichlet ^
  --pin_memory False ^
  --batch_size 4 ^
  --img_size 32 ^
  --num_workers 0 ^
  --train_data_ratio 1 ^
  --val_data_ratio 0 ^
  --val_dataset 0 ^
  --n_clients 20 ^
  --n_comm_rounds 200 ^
  --local_n_epochs 2 ^
  --world_conf 0,0,1,1,100 ^
  --on_cuda True ^
  --optimizer sgd ^
  --lr 0.1 ^
  --lr_warmup False ^
  --lr_scheduler MultiStepLR ^
  --lr_decay 0.1 ^
  --lr_milestones 133,167 ^
  --weight_decay 1e-4 ^
  --use_nesterov False ^
  --momentum_factor 0.9 ^
  --low_rank False ^
  --pruning True ^
  --dynamic False ^
  --freeze_bn True ^
  --need_scaler True ^
  --warmup_rounds 0 ^
  --track_time True ^
  --display_tracked_time True ^
  --python_path "%PYTHON_EXE%" ^
  --hostfile hostfile ^
  --manual_seed 0 ^
  --pn_normalize True ^
  --same_seed_process True ^
  --experiment %EXPERIMENT% ^
  --data cifar10 ^
  --non_iid_alpha 0.1 ^
  --participation_ratio 0.25 ^
  --group_norm_num_groups 0 ^
  --unit False ^
  --split_mix True ^
  --fl_aggregate scheme=federated_average ^
  --self_distillation 0 ^
  --global_rate 1

set "EXIT_CODE=%ERRORLEVEL%"
echo.
echo Process finished with exit code %EXIT_CODE%.
pause
exit /b %EXIT_CODE%